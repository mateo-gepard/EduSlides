      ...
      {bars.map((bar, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
          ...
          <motion.div
            ...
            style={{ background: `linear-gradient(to top, ${bar.color}40, ${bar.color})` }}
            initial={{ height: 0 }}
            animate={{ height: `${bar.percent}%` }}
            transition={{ delay: 0.5 + i * 0.1, duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
          />
          {/* Label */}
          <span className="text-[10px] text-slate-400 text-center leading-tight mt-1 max-w-[80px] truncate">
            {bar.label}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="flex items-center justify-center gap-8 h-full">
      <svg viewBox="0 0 240 240" className="w-60 h-60">
        ...
        {content.centerLabel && (
          <text x={cx} y={cy + 5} textAnchor="middle" fill="#e2e8f0" fontSize="12" fontWeight="700">
            {content.centerLabel}
          </text>
        )}
      </svg>
  ...
  return (
    <div className="flex flex-col h-full px-10 py-10">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4">
        ...
        {content.source && <p className="text-[10px] text-slate-600 mt-1">{content.source}</p>}
      </motion.div>
